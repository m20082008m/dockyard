...
func (r *radosdesc) New() (*radosdesc, error) {
...
func (r *radosdesc) Save(file string) (string, error) {
	...
	if _, err = r.WriteStream(file, 0, fp); err != nil {
		return "", err
	}
...
func (r *radosdesc) Put(path string, contents []byte) error {
	if _, err := r.WriteStream(path, 0, bytes.NewReader(contents)); err != nil {
		return err
	}
...
func (r *radosdesc) ReadStream(path string, offset int64) (io.ReadCloser, error) {
	// get oid from filename
	oid, err := r.getOid(path)
	if err != nil {
		return nil, err
	}
...
func (r *radosdesc) WriteStream(path string, offset int64, reader io.Reader) (totalRead int64, err error) {
	...
	if err != nil {
		...
		totalSize, err := r.getXattrTotalSize(oid)
		if err != nil {
			return 0, err
		}
		...
		for totalSize < uint64(offset) {
			sizeToWrite := r.Chunksize
			if totalSize-uint64(offset) < sizeToWrite {
				sizeToWrite = totalSize - uint64(offset)
			}

			chunkName, chunkOffset := r.getChunkNameFromOffset(oid, uint64(totalSize))
			err = r.Ioctx.Write(chunkName, buf[:sizeToWrite], uint64(chunkOffset))
			if err != nil {
	...
	for {
		// Align to chunk size
		sizeRead := uint64(0)
		sizeToRead := uint64(offset+totalRead) % r.Chunksize
		if sizeToRead == 0 {
			sizeToRead = r.Chunksize
		}
		...
		// Write chunk object
		chunkName, chunkOffset := r.getChunkNameFromOffset(oid, uint64(offset+totalRead))
		err = r.Ioctx.Write(chunkName, buf[:sizeRead], uint64(chunkOffset))
		...
		err = r.setXattrTotalSize(oid, uint64(offset+totalRead)+sizeRead)
		if err != nil {
			return totalRead, err
		}
...
func (r *radosdesc) Stat(path string) (FileInfo, error) {
	// get oid from filename
	oid, err := r.getOid(path)
	if err != nil {
		return nil, err
...
func (r *radosdesc) Delete(objectPath string) error {
	// Get oid
	oid, err := r.getOid(objectPath)
	if err != nil {
		return err
	}
	...
	if oid == "" {
		objects, err := r.listDirectoryOid(objectPath)
		if err != nil {
			return err
		}

		for object := range objects {
			err = r.Delete(path.Join(objectPath, object))
			if err != nil {
				return err
			}
		}
		...
		for offset := uint64(0); offset < totalSize; offset += r.Chunksize {
			chunkName, _ := r.getChunkNameFromOffset(oid, offset)

			err = r.Ioctx.Delete(chunkName)
			if err != nil {
				return err
			}
		}
...
func (r *radosdesc) generateOid() string {
	return objectBlobPrefix + Generate().String()
}

// Reference a object and its hierarchy
func (r *radosdesc) putOid(objectPath string, oid string) error {
	...
	if oid == "" {
		firstReference, err := r.Ioctx.GetOmapValues(directory, "", "", 1)
	...
	// Reference object
	err := r.Ioctx.SetOmap(directory, oids)
	if err != nil {
		return err
	}
	...
	if createParentReference {
		return r.putOid(directory, "")
	}

	return nil
}

func (r *radosdesc) getOid(objectPath string) (string, error) {
	directory := path.Dir(objectPath)
	base := path.Base(objectPath)

	files, err := r.Ioctx.GetOmapValues(directory, "", base, 1)
...
func (r *radosdesc) listDirectoryOid(path string) (list map[string][]byte, err error) {
	return r.Ioctx.GetAllOmapValues(path, "", "", defaultKeysFetched)
}

func (r *radosdesc) listObjects() ([]string, error) {
	objectList := []string{}
	err := r.Ioctx.ListObjects(func(oid string) {
		objectList = append(objectList, oid)
	})
	return objectList, err
}

func (r *radosdesc) deleteObject(oid string) error {
	return r.Ioctx.Delete(oid)
}

// Remove a file from the files hierarchy
func (r *radosdesc) deleteOid(objectPath string) error {
	// Remove object reference
	directory := path.Dir(objectPath)
	base := path.Base(objectPath)
	err := r.Ioctx.RmOmapKeys(directory, []string{base})
	if err != nil {
		return err
	}
...
func (r *radosdesc) getChunkNameFromOffset(oid string, offset uint64) (string, uint64) {
	chunkID := offset / r.Chunksize
	chunkedOid := oid + "-" + strconv.FormatInt(int64(chunkID), 10)
	chunkedOffset := offset % r.Chunksize

	return chunkedOid, chunkedOffset
}

// Set the total size of a chunked object `oid`
func (r *radosdesc) setXattrTotalSize(oid string, size uint64) error {
	...
	return r.Ioctx.SetXattr(oid+"-0", defaultXattrTotalSizeName, xattr)
}

func (r *radosdesc) getXattrTotalSize(oid string) (uint64, error) {
	// Fetch xattr as []byte
	xattr := make([]byte, binary.MaxVarintLen64)
	xattrLength, err := r.Ioctx.GetXattr(oid+"-0", defaultXattrTotalSizeName, xattr)
	if err != nil {
		return 0, err
	}
